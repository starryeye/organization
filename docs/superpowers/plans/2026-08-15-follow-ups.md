...
- **주기적 대조·자동 수렴** — 이번 장치는 *누군가 다시 건드린* 리소스의 어긋남만 그
  연산 범위에서 고친다. 아무도 다시 건드리지 않는 튜플의 어긋남은 여전히 발견되지 않고,
  발견돼도 전체를 훑어 자동으로 고치는 수단은 없다 — 사람이 재적재를 실행해야 한다.
  후보였던 표본 검증(§6 하단 "후보 목록 정정")은 이번 범위에 넣지 않았다.
- **스냅샷 의미 통일** — 바로 아래 "딸린 불일치"가 지적하는 SCIM(의도한 튜플) vs
  LDAP(실제 반영된 튜플) 불일치는 그대로다(설계 §5.5). 스냅샷을 쓰는 곳이 쓰기 경로가
  아니라 일일 아카이빙이라 이번 변경의 범위 밖에 뒀다.
