...
> **이 문서의 수치는 2026-09-07 회차의 것이고, 그 뒤 픽스처가 커졌다.**
> AD 범위 검색(`2026-09-09-ad-range-retrieval-design.md`)을 재현하려고 최대 조직을
> 500명 → 1,600명으로 키웠고, 직원 5,024 → **6,124** / 튜플 5,541 → **6,641** 이 됐다.
> `bootRun` + curl 로 손으로 잰 회차라 다시 돌리지 않았다 — **절차는 그대로 유효하고,
> 숫자만 옛 규모다.** 새 규모의 수치는 자동화 측정
> (`2026-09-06-scale-e2e-scenarios.md` §실측 비용)에 있다.
...
- ~~SCIM 쪽 실측은 아직이다.~~ **했다** — `docker/scim/replay.py` 로 실제 앱에 요청을 쏘고
  엔드포인트로 확인했다. 결과는 `2026-09-06-scale-e2e-scenarios.md` §4.5 에 있다(그것도
  옛 규모 회차다).
- **위 절차를 새 픽스처로 다시 돌리기.** 특히 조직 먼저 순서의 18배 지연(시나리오 문서
  §실측 비용)은 자동화로만 쟀다.
