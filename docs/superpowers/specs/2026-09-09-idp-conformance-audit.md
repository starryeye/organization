...
| L-1 | ~~AD 범위 검색 미지원~~ | ~~치명~~ | **고침 (2026-09-09)** — 아래 §3.1 참조 |

**S-1 과 L-1 이 이번 감사의 수확이다.** 둘 다 우리 테스트로는 영영 안 잡혔다 — 요청을
우리가 만들고, 픽스처 최대 그룹이 500명이었기 때문이다. (L-1 을 고치면서 그 그룹을
1,600명으로 키웠다. 그래도 임베디드 서버는 범위 검색을 하지 않으므로, 실제 AD 를 붙이기
전까지는 여전히 못 잡는다.)
...
### 3.1 L-1 · 범위 검색(range retrieval) — **고침**

AD 는 다중값 속성이 `MaxValRange`(1,000/1,500/5,000)를 넘으면 **속성 이름 자체를 바꿔서**
돌려준다 — `member` 를 값 없이, `member;range=0-1499` 를 값과 함께. 우리는 `member` 를
정확한 이름으로 읽었으므로 **빈 쪽**을 읽고 끝냈다.

**6,000명짜리 그룹이 멤버 0명으로 읽히고, 그 6,000개의 튜플이 삭제 대상이 된다.** 삭제
가드가 잡을 수도 있지만 전체가 10만이면 6% 라 그냥 지워진다. 그리고 서버는 정상 응답했고
우리는 정상적으로 읽었다고 믿으므로 **어디에도 오류가 남지 않는다.**

**우리 테스트로는 영영 안 잡혔다** — 픽스처 최대 조직이 500명이었고 임베디드 UnboundID 는
범위 검색을 하지 않는다. 임베디드 서버가 실제 서버보다 관대했던 **세 번째** 사례다.

**DIT 전략은 해당 없다.** dn 경로로 소속을 도출하므로 다중값 속성을 안 읽는다.

> **설계와 구현, 접근안 비교, 검증 한계는 별도 문서에 있다 —
> [`2026-09-09-ad-range-retrieval-design.md`](2026-09-09-ad-range-retrieval-design.md).**
>
> 요점만: 범위를 이어받되 **커넥션 하나 안에서** 끝낸다(순서 보장이 커넥션 단위다).
> 픽스처 최대 조직을 1,600명으로 키우고 이름을 "제1공장" 으로 바로잡았다.
> **실제 AD 로는 검증하지 못했다** — 합성 응답으로 "명세대로라면 맞다" 까지만 확인했다.
...
1. ~~**L-1 (AD 범위 검색)**~~ — **완료 (2026-09-09).** 다만 실제 AD 검증은 못 했다.
...
- **L-1 수정이 실제 AD 에서 정말 동작하는가** — 합성 응답으로만 검증했다
