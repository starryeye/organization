...
**전부 불일치는 그 회차를 실패시킨다.** 조직이 하나라도 있고 `member` 값이 하나 이상 있는데 **단 하나도**
대조되지 않으면, 사용자 DN 형태와 `member` 값의 형태가 어긋난 것이다. H 가 정확히 이 모양이었다.
...
**알면서 받아들이는 오탐 하나:** 사용자 검색 베이스를 좁게 잡아 그룹 멤버가 **전부 범위 밖**인 설정은 고장이
아닌데도 막힌다. 그 상태의 결과가 "권한 0" 이라 어차피 사람이 봐야 하므로 받아들인다. 설정 스위치는 두지
않는다 — 지금 필요하다는 근거가 없다.
