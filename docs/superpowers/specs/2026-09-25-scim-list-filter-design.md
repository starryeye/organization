...
## 11. 결과 (2026-09-25, DynamoDB Local)

**10만 명 목록 규모 테스트(`ScimListingScaleTest`)** — 두 번 돌렸다(구현 직후, 머지 전 전체 검증).

| | 1회 | 2회 |
|---|---|---|
| 심기(저장소 직접, 10만 명 + 5천 명 조직도) | 40.0초 | 31.7초 |
| Okta 식 가져오기(100명씩 1,000페이지) | 14.4초 | 12.7초 |
| 그 가져오기의 Query / 훑은 아이템 / GetItem | 1,017번 / **200,000** / 999번 | (같은 코드) |
| 조직을 멤버까지 끝까지 | 0.16초 | 0.24초 |
| 무작위 100명 × (`userName` 대문자, `externalId`) | 0.49초 | 0.55초 |

- **훑은 아이템이 정확히 2N(200,000)이다** — 첫 페이지의 COUNT 가 N, 페이지들이 N. 다 읽고 자르기였다면 N²/100 = 1억이다.
  GetItem 999번은 둘째 페이지부터의 책갈피 조회 하나씩이다.
- 필터 조회는 한 번에 GSI1 1건, 또는 GSI3 1건 + GetItem 1번만 읽었다.

**머지 전 전체 검증(b7c0cab)** — `./gradlew cleanTest test` 2분 6초, `./gradlew cleanScaleTest scaleTest` 11분 21초,
둘 다 통과. 머신 부하가 높았다(load average 10 안팎).

**§8 과 다르게 한 것** — "조직을 멤버까지 끝까지 가져와 조직도와 대조" 는 5천 명 SCIM 시나리오가 아니라 10만 명 규모
테스트 안에서 한다(같은 5천 명 조직도를 저장소에 직접 심는다). `excludedAttributes=members` 가 멤버를 읽지 않음은 E2E 의
스파이 대신 connector-scim 단위 테스트의 `findGroupCalls` 로 확인한다(계획 앞머리).

**실행 중 바뀐 것** — 필터의 `true`/`false` 는 소문자만 받는다(RFC 7644 compValue 는 JSON 리터럴 규칙, §4.1 의 "대소문자
무시" 는 `and`·`eq`·속성 이름뿐). 조직의 쓰기 응답(POST·PUT·PATCH)도 `members` 를 빼면 이름표만 읽는다. 책갈피 읽기·쓰기가
실패해도 페이지는 준다(건너뛰기로 대신한다). 테이블 TTL(`expiresAt`)은 책갈피만이 아니라 스냅샷·동기화 이력·쓰기 락에도
걸린다 — 원래 설계가 그 속성을 두고 기다리던 것이다(README).
